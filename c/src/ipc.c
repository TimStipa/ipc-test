...
#define Q_NAME    "/ipc_test"
#define MAX_SIZE  1024
#define M_EXIT    "done"
#define SERVER_FLAG  "-producer"
#define CLIENT_FLAG  "-consumer"

int main(int argc, char *argv[])
{
    if (argc != 2)
    {
        printf("Unspecified configuration");
    }
    else
    {	    
        if (0 == strncmp(argv[1], SERVER_FLAG, strlen(SERVER_FLAG)))
        {
            producer();
        }
	else if (0 ==strncmp(argv[1], CLIENT_FLAG, strlen(CLIENT_FLAG)))
	{
	    consumer();
	}	
        else
        {
            printf("Unknown task specified.");
        }
    }	
}    
